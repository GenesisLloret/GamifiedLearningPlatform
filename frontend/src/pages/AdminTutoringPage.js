...
function AdminTutoringPage() {
  ...
  useEffect(() => { fetchUsers(); fetchTutoring(); }, []);
  const handleTutorChange = e => setTutorData({ ...tutorData, [e.target.name]: e.target.value });
  const handleAssignTutoring = async e => {
    ...
    const data = await res.json();
    if (res.ok) { setMsg('Tutoría asignada'); setTutorData({ professorId: '', studentId: '' }); fetchTutoring(); }
    else setMsg(data.msg || 'Error al asignar tutoría');
  };
  const handleDeleteTutoring = async id => {
    ...
    const data = await res.json();
    if (res.ok) { setMsg('Relación eliminada'); fetchTutoring(); }
    else setMsg(data.msg || 'Error al eliminar tutoría');
  };
  return (
